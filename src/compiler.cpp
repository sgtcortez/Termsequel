...
void append_action(const Termsequel::Lexeme *lexeme,
  ...
  case lexeme_key::COLUMN:
...

    {
      ...
      if (lexeme::NAME == lexeme->get_type())
        column = column_type::FILENAME;
      else if (lexeme::SIZE == lexeme->get_type())
        column = column_type::FILESIZE;
      else if (lexeme::OWNER == lexeme->get_type())
        column = column_type::OWNER;
#ifdef __linux__
      else if (lexeme::GROUP == lexeme->get_type())
        column = column_type::GROUP;
#endif
      else if (lexeme::LEVEL == lexeme->get_type())
        column = column_type::LEVEL;
      else if (lexeme::FILE_TYPE == lexeme->get_type())
        column = column_type::FILE_TYPE;
      else if (lexeme::OWNER_PERMISSIONS == lexeme->get_type())
        column = column_type::OWNER_PERMISSIONS;
#ifdef __linux__
      else if (lexeme::GROUP_PERMISSIONS == lexeme->get_type())
        column = column_type::GROUP_PERMISSIONS;
      else if (lexeme::OTHERS_PERMISSIONS == lexeme->get_type())
      ...
      else if (lexeme::CREATION_DATE == lexeme->get_type())
        column = column_type::CREATION_DATE;
      else if (lexeme::RELATIVE_PATH == lexeme->get_type())
        column = column_type::RELATIVE_PATH;
      else if (lexeme::ABSOLUTE_PATH == lexeme->get_type())
        column = column_type::ABSOLUTE_PATH;
  ...
  case lexeme_key::COMPARASION: {
    ...
    if (lexeme::EQUAL == lexeme->get_type())
      operator_value = operator_key::EQUAL;
    else if (lexeme::NOT_EQUAL == lexeme->get_type())
      operator_value = operator_key::NOT_EQUAL;
    else if (lexeme::STARTS_WITH == lexeme->get_type())
      operator_value = operator_key::STARTS_WITH;
    else if (lexeme::NOT_STARTS_WITH == lexeme->get_type())
      operator_value = operator_key::NOT_STARTS_WITH;
    else if (lexeme::ENDS_WITH == lexeme->get_type())
      operator_value = operator_key::ENDS_WITH;
    else if (lexeme::NOT_ENDS_WITH == lexeme->get_type())
      operator_value = operator_key::NOT_ENDS_WITH;
    else if (lexeme::CONTAINS == lexeme->get_type())
      operator_value = operator_key::CONTAINS;
    else if (lexeme::NOT_CONTAINS == lexeme->get_type())
      operator_value = operator_key::NOT_CONTAINS;
    else if (lexeme::BIGGER == lexeme->get_type())
      operator_value = operator_key::BIGGER;
    else if (lexeme::LESS == lexeme->get_type())
      operator_value = operator_key::LESS;
    else if (lexeme::BIGGER_OR_EQUAL == lexeme->get_type())
      operator_value = operator_key::BIGGER_OR_EQUAL;
    else if (lexeme::LESS_OR_EQUAL == lexeme->get_type())
      operator_value = operator_key::LESS_OR_EQUAL;
    else {
      std::cerr << "Unmapped operator value!" << std::endl;
      break;
    }
    system_command->conditions
        ->conditions[system_command->conditions->conditions.size() - 1]
  ...
  case lexeme_key::LOGICAL:
    if (lexeme::AND == lexeme->get_type()) {
      system_command->conditions->operators.push_back(
          Termsequel::LogicalOperator::AND);
    } else {
      system_command->conditions->operators.push_back(
