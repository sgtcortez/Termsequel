...
static int termsequel_stat(const char *filename, stat_buffer_t *stat_buffer) {
  ...
  return syscall(__NR_statx, AT_FDCWD, filename, AT_SYMLINK_NOFOLLOW, STATX_ALL,
                 stat_buffer);
}
 ...
 * ... This is why, we need to use this version of stat, and, use if-elif
 * directives ...
 */
...
get_directory_information(std::string name,
                          ...
                          std::uint16_t current_level, std::uint16_t max_depth);
...
static bool should_go_recursive(std::uint16_t current_level,
...
bool compare(bool left, bool right, Termsequel::LogicalOperator operator_type);
...
bool Termsequel::System::execute(const Termsequel::SystemCommand *command,
  ...
  const auto stat_array =
      get_information(target, command->conditions, INITIAL_LEVEL, max_depth);
  ...
  for (const auto column : command->columns) {
    ...
    if (column == COLUMN_TYPE::LAST_MODIFICATION) {
      ...
      header.insert(header.end(),
                    bigger_last_modification - strlen("Last Modification"),
                    ' ');
    }
    if (column == COLUMN_TYPE::CREATION_DATE) {
      header.append(" Creation Date");
      header.insert(header.end(),
                    bigger_creation_date - strlen("Creation Date"), ' ');
    }
    if (column == COLUMN_TYPE::RELATIVE_PATH) {
      header.append(" Relative Path");
      header.insert(header.end(),
                    bigger_relative_path - strlen("Relative Path"), ' ');
    }
    if (column == COLUMN_TYPE::ABSOLUTE_PATH) {
      header.append(" Absolute Path");
      header.insert(header.end(),
                    bigger_absolute_path - strlen("Absolute Path"), ' ');
    }
  }
  ...
  for (const auto stat_element : *stat_array) {
    ...
    for (const auto column : command->columns) {
      ...
      if (column == COLUMN_TYPE::LAST_MODIFICATION) {
        ...
        string.insert(string.end(),
                      bigger_last_modification -
                          stat_element->last_modification.size(),
                      ' ');
      }
      if (column == COLUMN_TYPE::CREATION_DATE) {
        string.push_back(' ');
...
get_information(std::string name, Termsequel::ConditionList *conditions,
  ...
  stat_value->permissions.owner[0] =
      get_mode(stat_buffer) & READ_PERMISSION_OWNER ? 'R' : '-';
  stat_value->permissions.owner[1] =
      get_mode(stat_buffer) & WRITE_PERMISSION_OWNER ? 'W' : '-';
  stat_value->permissions.owner[2] =
      get_mode(stat_buffer) & EXECUTE_PERMISSION_OWNER ? 'X' : '-';
  stat_value->permissions.owner[3] = '\0';
#ifdef __linux__
  stat_value->permissions.group[0] =
      get_mode(stat_buffer) & READ_PERMISSION_GROUP ? 'R' : '-';
  stat_value->permissions.group[1] =
      get_mode(stat_buffer) & WRITE_PERMISSION_GROUP ? 'W' : '-';
  stat_value->permissions.group[2] =
      get_mode(stat_buffer) & EXECUTE_PERMISSION_GROUP ? 'X' : '-';
  stat_value->permissions.group[3] = '\0';

  stat_value->permissions.others[0] =
      get_mode(stat_buffer) & READ_PERMISSION_OTHERS ? 'R' : '-';
  stat_value->permissions.others[1] =
      get_mode(stat_buffer) & WRITE_PERMISSION_OTHERS ? 'W' : '-';
  ...
  std::strftime(buffer, sizeof(buffer) / sizeof(buffer[0]), "%Y-%m-%dT%H:%M:%S",
                date);
  ...
  if (error_number != 0) {
    // error here
    std::cerr << "Could not get the locatime. Error Number: " << error_number
              << std::endl;
  }
  std::strftime(buffer, sizeof(buffer) / sizeof(buffer[0]), "%Y-%m-%d %H:%M:%S",
                &local_time);
  ...
  DWORD return_code =
      GetFullPathName(name.c_str(), BUFFER_SIZE, absolute_path_buffer, NULL);
  if (return_code == 0) {
...
static std::vector<struct StatResult *> *get_directory_information(
    std::string name, Termsequel::ConditionList *conditions,
    std::uint16_t current_level, std::uint16_t max_depth) {
  ...
  while (true) {
    ...
    auto info_vector = get_information(relative_path, conditions,
        ...
        if (!compare_string) {
          ...
          current =
              std::strncmp(compare_value.string_value, condition->value.c_str(),
                           strlen(compare_value.string_value)) == 0;
        }
        break;
